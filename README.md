...
- 처음 접속하면 영어로 시작하며, 메뉴 마지막의 `KO` / `EN` 버튼으로 언어 전환
- 선택한 언어는 탭의 `sessionStorage`에 저장하여 같은 탭에서 새로고침해도 유지
- 새 세션에서는 다시 영어로 시작하며, 브라우저에서 저장소를 차단한 경우 현재 화면에서만 언어 전환 가능
- 모바일에서는 펼친 메뉴의 마지막에 언어 전환 버튼이 표시됨
- 영문 원본은 `content.js`, 한국어 번역은 `translations.js`에서 수정
- 번역은 `"영문 원문": "한국어 번역"` 형태이므로 영문 문구를 바꾸면 번역의 왼쪽 원문도 함께 수정
- 같은 영문 문구는 번역 한 항목으로 공유하며, 미번역 문구는 영문으로 표시
- 연사 이름과 기관 표기도 `translations.js`에서 필요에 따라 번역 가능
- 한국어 번역만 바꿨다면 `index.html`의 `translations.js?v=` 숫자를 올려 캐시 갱신
- 한국어 전용 글자 크기·서체는 `styles.css`의 `html[lang="ko"]` 규칙에서 조절하며 영어 스타일에는 영향을 주지 않음

## 파일 구성

- index.html: 페이지의 기본 뼈대
- content.js: 워크숍 정보와 반복 콘텐츠
- translations.js: 한국어 번역
